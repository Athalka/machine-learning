...
def get_google_trend_data(kw_list=None,start_from=None,years=1 ,filename=None, csv_output=False, ff_to_today=False):
    path = '.'
    os.chdir(path)

    if filename is None:
        filename = 'googletrends.csv'
    ...
    if kw_list is None:
        kw_list = ["dolar kuru","ekonomi", "kriz","banka"]

    if start_from is None:
        #Check years  - defaults to 1 year ago
    ...
    pytrend.build_payload(kw_list=kw_list, geo="TR", timeframe = timeframe)
    ...
    if ff_to_today:
        least_recent_date = interest_over_time_df.index.min()
        recent_date = interest_over_time_df.index.max()

        ix = pd.DatetimeIndex(start=least_recent_date, end=datetime.today(), freq='D')
        interest_over_time_df = interest_over_time_df.reindex(ix,method="ffill",fill_value=0)
